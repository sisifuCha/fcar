...
# 返回电池电压 — 2字节无符号大端序，单位0.01V
def return_battery_voltage(tcp):
    T_CARTYPE = g_car_type
    T_FUNC = 0x02
    T_LEN = 0x04
    vol_raw = int(g_bot.get_battery_voltage() * 100)  # e.g. 7.50V → 750
    if vol_raw < 0:
        vol_raw = 0
    vol_bytes = bytearray(struct.pack('>H', vol_raw))  # 2字节，大端序
    checknum = (T_CARTYPE + T_FUNC + T_LEN + vol_bytes[0] + vol_bytes[1]) % 256
    data = "$%02x%02x%02x%02x%02x%02x#" % (T_CARTYPE, T_FUNC, T_LEN, vol_bytes[0], vol_bytes[1], checknum)
    tcp.send(data.encode(encoding="utf-8"))
    if g_debug:
        print("voltage:", vol_raw / 100.0)
        print("tcp send:", data)
...
# 返回IMU姿态数据 (roll, pitch, yaw) — 度*100, 有符号16位
def return_imu_attitude(tcp):
    T_CARTYPE = g_car_type
    T_FUNC = 0x03
    roll, pitch, yaw = g_bot.get_imu_attitude_data(ToAngle=True)
    r = int(roll * 100)
    p = int(pitch * 100)
    y = int(yaw * 100)
    r_bytes = bytearray(struct.pack('h', r))
    p_bytes = bytearray(struct.pack('h', p))
    y_bytes = bytearray(struct.pack('h', y))
    T_LEN = 0x08  # 6 data bytes + 2
    checknum = (T_CARTYPE + T_FUNC + T_LEN +
                r_bytes[0] + r_bytes[1] + p_bytes[0] + p_bytes[1] + y_bytes[0] + y_bytes[1]) % 256
    data = "$%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x#" % (
        T_CARTYPE, T_FUNC, T_LEN,
        r_bytes[0], r_bytes[1], p_bytes[0], p_bytes[1], y_bytes[0], y_bytes[1],
        checknum)
    tcp.send(data.encode(encoding="utf-8"))
    if g_debug:
        print("imu attitude: roll=%.2f, pitch=%.2f, yaw=%.2f" % (roll, pitch, yaw))
        print("tcp send:", data)


# 返回运动速度数据 (vx, vy, vz) — m/s*1000, 有符号16位
def return_motion_speed(tcp):
    T_CARTYPE = g_car_type
    T_FUNC = 0x04
    vx, vy, vz = g_bot.get_motion_data()
    vx_val = int(vx * 1000)
    vy_val = int(vy * 1000)
    vz_val = int(vz * 1000)
    vx_bytes = bytearray(struct.pack('h', vx_val))
    vy_bytes = bytearray(struct.pack('h', vy_val))
    vz_bytes = bytearray(struct.pack('h', vz_val))
    T_LEN = 0x08  # 6 data bytes + 2
    checknum = (T_CARTYPE + T_FUNC + T_LEN +
                vx_bytes[0] + vx_bytes[1] + vy_bytes[0] + vy_bytes[1] + vz_bytes[0] + vz_bytes[1]) % 256
    data = "$%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x#" % (
        T_CARTYPE, T_FUNC, T_LEN,
    ...
    tcp.send(data.encode(encoding="utf-8"))
    if g_debug:
        print("motion speed: vx=%.3f, vy=%.3f, vz=%.3f" % (vx, vy, vz))
        print("tcp send:", data)


# 返回电机编码器数据 (m1, m2, m3, m4) — 有符号32位
def return_encoder_data(tcp):
    T_CARTYPE = g_car_type
    T_FUNC = 0x05
    m1, m2, m3, m4 = g_bot.get_motor_encoder()
    m1_bytes = bytearray(struct.pack('i', m1))
    m2_bytes = bytearray(struct.pack('i', m2))
    m3_bytes = bytearray(struct.pack('i', m3))
    m4_bytes = bytearray(struct.pack('i', m4))
    T_LEN = 0x12  # 16 data bytes + 2
    checknum = (T_CARTYPE + T_FUNC + T_LEN +
                m1_bytes[0] + m1_bytes[1] + m1_bytes[2] + m1_bytes[3] +
                m2_bytes[0] + m2_bytes[1] + m2_bytes[2] + m2_bytes[3] +
                m3_bytes[0] + m3_bytes[1] + m3_bytes[2] + m3_bytes[3] +
                m4_bytes[0] + m4_bytes[1] + m4_bytes[2] + m4_bytes[3]) % 256
    data = "$%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x#" % (
        T_CARTYPE, T_FUNC, T_LEN,
        m1_bytes[0], m1_bytes[1], m1_bytes[2], m1_bytes[3],
        m2_bytes[0], m2_bytes[1], m2_bytes[2], m2_bytes[3],
        m3_bytes[0], m3_bytes[1], m3_bytes[2], m3_bytes[3],
        m4_bytes[0], m4_bytes[1], m4_bytes[2], m4_bytes[3],
        checknum)
    tcp.send(data.encode(encoding="utf-8"))
    if g_debug:
        print("encoder: m1=%d, m2=%d, m3=%d, m4=%d" % (m1, m2, m3, m4))
        print("tcp send:", data)
...
def parse_data(sk_client, data):
    ...
    elif cmd == "03":  # 获取IMU姿态数据
        if g_debug:
            print("get imu attitude")
        return_imu_attitude(sk_client)

    elif cmd == "04":  # 获取运动速度数据
        if g_debug:
            print("get motion speed")
        return_motion_speed(sk_client)

    elif cmd == "05":  # 获取编码器数据
        if g_debug:
            print("get encoder data")
        return_encoder_data(sk_client)
